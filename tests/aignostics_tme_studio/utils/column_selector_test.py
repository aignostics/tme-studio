...
TISSUE_STAT = data_classes.Statistic(name="Relative Area", formatter="RELATIVE_AREA_{tissue_cls}", unit="%")
CELL_IN_TISSUE_STAT = data_classes.Statistic(name="Density", formatter="DENSITY_OF_{cell_cls}_{tissue_cls}", unit="")


@pytest.fixture(name="dummy_df")
def create_dummy_df() -> pd.DataFrame:
    """Create a dummy dataframe with columns corresponding to the model output classes and statistics."""
    gen = np.random.default_rng(seed=42)
    columns = []
    for tissue_cls in MODEL_OUTPUT_CLASSES["tissue_cls"]:
        columns.extend([
            f"DENSITY_OF_{utils.to_allcaps(cell_cls)}_{utils.to_allcaps(tissue_cls)}"
            for cell_cls in MODEL_OUTPUT_CLASSES["cell_cls"]
        ])
        columns.append(f"RELATIVE_AREA_{utils.to_allcaps(tissue_cls)}")

    columns.extend([f"DENSITY_OF_{utils.to_allcaps(cell_cls)}" for cell_cls in MODEL_OUTPUT_CLASSES["cell_cls"]])
...
def test_column_selector_get_column_format(dummy_df) -> None:
    ...
    assert df.shape[1] == len(MODEL_OUTPUT_CLASSES["tissue_cls"])
...
def test_column_selector_multiple_formatters(dummy_df) -> None:
    """Test that get_column_format correctly fills in the formatter string."""
    selector = column_selector.FeatureColumnSelector(
        statistics=[CELL_IN_TISSUE_STAT],
        x_variable="cell_cls",
        model_output_class_config=MODEL_OUTPUT_CLASSES,
    )
    dropdowns = selector.render_dropdowns()

    # 1 dropdown for selecting statistic, 1 dropdown for selecting tissue type.
    assert len(dropdowns.value) == 2

    df = selector.extract_feature_columns(df=dummy_df, **dropdowns.value)
    assert len(df) == len(dummy_df)
    assert np.all([c in df.columns for c in MODEL_OUTPUT_CLASSES["cell_cls"]])
