...
def _features_to_dropdown(features: list[Feature], label: str) -> mo.ui.dropdown:
    """Create a dropdown from a features list.

    Args:
        features: The list of features (pretty-print) with names and associated dataframe column header formatting
            strings.
    ...
    return mo.ui.dropdown(
        options={feat.name: feat.formatter for feat in features},
        label=label,
        value=next(iter(features)).name,
    )
...
class FeatureColumnSelector:
    ...
    Helper class that is given a list of features, and provides a list of dropdowns to fill placeholders in the
    features. When the dropdown values are passed back to the class, it selects the appropriate TME feature columns
    by formatting the selected feature with the provided values.
    When a placeholder is selected as "x_variable", all possible values for that placeholder are used to select a set of
    feature columns.

    Example:
        Our features are ["DENSITY_{cell_cls}_IN_{tissue_cls}", "RATIO_{cell_cls}_IN_{tissue_cls}"].
        We set "cell_cls" to be our `x_variable`.
        1. **creating dropdowns**: The `FeatureColumnSelector` sees the `cell_cls` and `tissue_cls` placeholders
            and creates dropdowns. In this case we would create two dropdowns: one to select from the list of
            features, and one to select from all available tissue classes to fill the `tissue_cls` placeholder.
            Since "cell_cls" is our `x_variable`, no dropdown is created for `cell_cls`.
        2. **selecting columns**: The user selects feature: `"DENSITY_{cell_cls}_IN_{tissue_cls}"` and
            `tissue_cls`: `"Stroma"`. The `FeatureColumnSelector` formats the feature to
            `"DENSITY_{cell_cls}_IN_STROMA"` and will then loop over all possible values for `cell_cls`
            to find the set of columns:
            ["DENSITY_LYMPHOCYTES_IN_STROMA", "DENSITY_FIBROBLASTS_IN_STROMA", etc. ]
    """

    def __init__(self, features: list[Feature], x_variable: str, model_config: dict[str, list[str]]) -> None:
        """Init feature column selector section.

        Args:
            features: List of features to select from, with formatting strings containing placeholders, e.g.
                "density_{cell_cls}_in_{tissue_cls}".
                Note: each feature in the list should be of the same type, meaning, contain the same set
                of placeholders.
            x_variable: The placeholder that should be used as x variable in the plot, and for which the columns
                should be found by formatting with all possible values. Example: "cell_cls"; in which case columns
                will be found for all possible cell classes.
            model_config: Dictionary mapping placeholders to possible values, used to format the feature
    ...
    def _create_column_mapping(self, **dropdown_args) -> dict[str, str]:
        ...
        """
        # Format the feature with the variables selected with the dropdowns
        column_format = self.get_column_format(dropdown_args)

        # Create one column mapping for each value available for `x_variable`
        if self.x_variable:
            x_values = self._get_model_variables_for_placeholder(self.x_variable)
    ...
    def _get_model_variables_for_placeholder(self, placeholder: str) -> list[str]:
        ...
        if placeholder not in self.model_config:
            placeholder = next(iter([k for k in self.model_config if placeholder.startswith(k)]))
        return self.model_config[placeholder]

    def _extract_placeholders(self) -> list[str]:
        """Extract placeholders that are in the (list of) feature(s).
    ...
    def get_column_format(self, dropdown_args: dict[str, str]) -> str:
        """Get feature formatter string with all placeholders filled except for `x_variable`.

        Args:
            dropdown_args: Values selected by the user, keyed by dropdown name.
                The "feat" key is consumed to retrieve the formatter string.
        ...
        """
        # Extract the formatter string for the feature to plot, e.g. "density_{cell_cls}"
        feature = dropdown_args.pop("feat")
        ...
        return feature.format(**format_args)
...
class CellInTissueFeatureColumnSelector(NoAnucleatedAreasFeatureColumnSelector):
    ...
    This extractor extracts Cells-inside-tissue features.

    - Cells-in-tissue features are not available for anucleated areas and thus these must be filtered out.
