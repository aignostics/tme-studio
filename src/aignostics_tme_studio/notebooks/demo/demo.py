...
def _(mo):
    ...
    This report presents an overview of Atlas H&E-TME analysis results for all whole slide images (WSIs) in TCGA.

    The tables and figures in this report present a selected subset of the Atlas H&E-TME readouts.
    The readouts, which capture tumor microenvironment (TME) statistics and features, are produced through
    sequential inference of tissue quality control (QC), cell detection (CD), tissue segmentation (TS),
...
def _(mo):
    ...
    OpenTME is loaded from [Hugging Face](https://huggingface.co/datasets/Aignostics/OpenTME) 🤗
    To obtain access, do the following:
    ...
    2. Request access to OpenTME. Go to https://huggingface.co/datasets/Aignostics/OpenTME and click "Access". You will
       receive an email from Hugging Face as soon as your access request has been reviewed. This may take a few working
       days.
    3. Create an access token by going to [hf.co/settings/tokens](https://hf.co/settings/tokens).
...
def _(hf_token, mo, pd):
    ...
    try:
        path = hf_hub_download(
            ...
            filename=hf_files.TISSUE_FEATURES_FILES.format("bladder_cancer"),
            repo_type="dataset",
            token=token,
        )
    ...
    _res
    return df, df_meta, errors, hf_files, hf_hub_download, token
...
@app.cell(hide_code=True)
def _(errors, hf_files, hf_hub_download, token):
    # Download the model output class settings and lists of available features.
    from aignostics_tme_studio.utils import column_selector, utils
